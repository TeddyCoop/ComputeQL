...
pg_oid_for_column_type(GDB_ColumnType type)
{
  switch (type)
  {
    ...
    case GDB_ColumnType_Bool:    return 16;  // bool
    case GDB_ColumnType_I32:     return 23;  // int4
    case GDB_ColumnType_I64:     return 20;  // int8
...
pg_typlen_for_column_type(GDB_ColumnType type)
{
  switch (type)
  {
    ...
    case GDB_ColumnType_Bool: return 1;
    case GDB_ColumnType_I32:  return 4;
    case GDB_ColumnType_I64:  return 8;
...
{
  U8 len_bytes[4];
  if (!os_net_recv_exact(conn, len_bytes, 4)) 
  { 
    return 0;
  }
  ...
  if (len < 4 || len > MB(1)) 
  { 
    // tec: cap against a hostile length
    return 0;
  } 
  
  U64 body_size = len - 4;
  U8* body = push_array_no_zero(arena, U8, Max(body_size, 1));
  if (body_size > 0 && !os_net_recv_exact(conn, body, body_size))
  { 
    return 0; 
  }
...
{
  U8 type = 0;
  if (!os_net_recv_exact(conn, &type, 1)) 
  {
    return 0; 
  }
  
  String8 body = {0};
  if (!pg_recv_untyped(arena, conn, &body)) 
  { 
    return 0; 
  }
...
pg_encode_binary_numeric(Arena* arena, GDB_ColumnType type, F64 value)
{
  switch (type)
  {
    ...
    {
      U8* b = push_array_no_zero(arena, U8, 1);
      b[0] = (U8)value;
      return str8(b, 1);
    }
...
internal B32 
pg_send_no_data(OS_Handle conn, Arena* arena)         
{ 
  return pg_send_msg(conn, arena, 'n', 0);
}

internal B32
pg_send_parse_complete(OS_Handle conn, Arena* arena)
{
  return pg_send_msg(conn, arena, '1', 0); 
}
internal B32 
pg_send_bind_complete(OS_Handle conn, Arena* arena)
{ 
  return pg_send_msg(conn, arena, '2', 0);
}

internal B32
pg_send_close_complete(OS_Handle conn, Arena* arena)
{
  return pg_send_msg(conn, arena, '3', 0);
}

internal B32 
pg_send_portal_suspended(OS_Handle conn, Arena* arena)
{
  return pg_send_msg(conn, arena, 's', 0); 
}
  ...
  if (format == 1)
  {
    switch (declared_oid)
    {
      case PG_OID_INT2:
      {
        if (raw_value.size != 2) 
        {
          *out_error = str8_lit("malformed binary int2 parameter");
          return 0;
        }
      ...
      case PG_OID_INT4:
      {
        if (raw_value.size != 4)
        { 
          *out_error = str8_lit("malformed binary int4 parameter"); 
          return 0; 
        }
      ...
      case PG_OID_INT8:
      {
        if (raw_value.size != 8) 
        { 
          *out_error = str8_lit("malformed binary int8 parameter");
          return 0;
        }
      ...
      case PG_OID_FLOAT4:
      {
        if (raw_value.size != 4) 
        { 
          *out_error = str8_lit("malformed binary float4 parameter"); 
          return 0; 
        }
      ...
      case PG_OID_FLOAT8:
      {
        if (raw_value.size != 8)
        { 
          *out_error = str8_lit("malformed binary float8 parameter"); 
          return 0;
        }
        U64 bits = ((U64)pg_read_be32(raw_value.str) << 32) | (U64)pg_read_be32(raw_value.str + 4);
      ...
      case PG_OID_BOOL:
      {
        if (raw_value.size != 1)
        { 
          *out_error = str8_lit("malformed binary bool parameter");
          return 0;
        }
      ...
      case PG_OID_TEXT: case PG_OID_VARCHAR: case PG_OID_BPCHAR: case PG_OID_UNKNOWN: case 0:
      {
        ...
        {
          if (raw_value.str[i] == 0) 
          { 
            *out_error = str8_lit("NUL bytes are not supported in parameter values");
            return 0; 
          }
        }
...
pg_substitute_params(Arena* arena, String8 sql_text, String8* literals, U32 literal_count)
{
  ...
  while (i < sql_text.size)
  {
    ...
    if (c == '$' && i + 1 < sql_text.size && char_is_digit(sql_text.str[i + 1], 10))
    {
      ...
      while (num_end < sql_text.size && char_is_digit(sql_text.str[num_end], 10))
      {
        num_end++; 
      }
