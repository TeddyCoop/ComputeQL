...
server_connection_thread_proc(void *ptr)
{
  ...
  log_info("server: connection opened");
  ...
  for (;;)
  {
    ...
    {
      log_error("server: rejecting request with invalid length %llu - closing connection", sql_len);
      break;
    }
    ...
    B32 send_ok = os_net_send_exact(socket, &status, sizeof(status));
    if (send_ok)
    {
      U64 payload_len = payload.size;
      send_ok = os_net_send_exact(socket, &payload_len, sizeof(payload_len));
      if (send_ok && payload_len > 0)
      {
        send_ok = os_net_send_exact(socket, payload.str, payload_len);
      }
    }
