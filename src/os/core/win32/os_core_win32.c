...
////////////////////////////////
//~ tec: @os_hooks System/Process Info (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Memory Allocation (Implemented Per-OS)
...
os_reserve(U64 size)
{
  ...
  if (result == 0)
  {
    ...
    log_error("VirtualAlloc (MEM_RESERVE) failed (size=%llu) - Code: %lu", size, error);
  }
  return result;
}
...
os_commit(void *ptr, U64 size)
{
  ...
  {
    DWORD error = GetLastError();
    log_error("VirtualAlloc failed (size=%llu) - Code: %lu", size, error);
  }
  return result;
}
...
{
  // NOTE(tec): size not used - not necessary on Windows, but necessary for other OSes.
...
////////////////////////////////
//~ tec: @os_hooks Thread Info (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Aborting (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks File System (Implemented Per-OS)
...
os_file_map_view_open(OS_Handle map, OS_AccessFlags flags, Rng1U64 range)
{
  ...
  */
  ...
  if (!GetFileSizeEx((HANDLE)map.u64[1], &file_size_li)) {
    log_error("GetFileSizeEx failed - Code: %lu", GetLastError());
    return 0;
  }
  U64 file_size = (U64)file_size_li.QuadPart;
  
  if (aligned_offset + aligned_size > file_size) {
    return 0;
  }
  
  U64 file_end = file_size;
  U64 requested_end = aligned_offset + aligned_size;
  
  if (requested_end > file_end)
  {
    aligned_size = file_end - aligned_offset;
    log_error("requested mapping range is out of bounds (file_size=%llu, requested_end=%llu)", file_size, aligned_offset + aligned_size);
    log_warn("truncating mapping to file size (aligned_size now %llu)", aligned_size);
  }
...
////////////////////////////////
//~ tec: @os_hooks Shared Memory (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Time (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Child Processes (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Threads (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Synchronization Primitives (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Dynamically-Loaded Libraries (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Safe Calls (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks GUIDs (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Entry Points (Implemented Per-OS)
...
w32_entry_point_caller(int argc, WCHAR **wargv)
{
  ...
  //- tec: do OS layer initialization
  {
    // tec: dynamically load windows functions which are not guaranteed
    // in all SDKs
    {
    ...
    // tec: try to enable large pages if we can
    {
      HANDLE token;
    ...
    // tec: set up non-dynamically-alloc'd state
    //
    ...
    // tec: set up thread context
    local_persist TCTX tctx;
    tctx_init_and_equip(&tctx);
    
    // tec: set up dynamically-alloc'd state
