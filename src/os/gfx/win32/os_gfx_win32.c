...
os_w32_wnd_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if(good)
  {
    ...
    switch(uMsg)
    {
      ...
      {
        // NOTE(tec): undocumented messages for drawing themed window borders.
        if(window != 0 && window->custom_border)
        {
          result = 0;
        }
      ...
      //- tec: [custom border] activation - without this `result`, stuff flickers.
      case WM_NCACTIVATE:
      {
      ...
      //- tec: [custom border] client/window size calculation
      case WM_NCCALCSIZE:
      if(window != 0)
      {
      ...
      //- tec: [custom border] client/window hit testing (mapping mouse -> action)
      case WM_NCHITTEST:
      {
        DWORD window_style = window ? GetWindowLong(window->hwnd, GWL_STYLE) : 0;
        B32 is_fullscreen = !(window_style & WS_OVERLAPPEDWINDOW);
        ...
        else
        {
          ...
          //- tec: check against window boundaries
          RECT frame_rect;
          ...
          //- tec: check against title bar client areas
          ...
          //- tec: resolve hovering to result
          result = HTNOWHERE;
          if(is_over_window)
          {
            // tec: default to client area
            result = HTCLIENT;
...
////////////////////////////////
//~ tec: @os_hooks Main Initialization API (Implemented Per-OS)

internal void
os_gfx_init(void)
{
  //- tec: set up base shared state
  ...
  //- tec: register graphical-window class
  {
  ...
  //- tec: set initial cursor
  os_set_cursor(OS_Cursor_Pointer);
  
  //- tec: fill vkey -> OS_Key table
  {
...
////////////////////////////////
//~ tec: @os_hooks Graphics System Info (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Clipboards (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Windows (Implemented Per-OS)
...
os_window_open(Vec2F32 resolution, OS_WindowFlags flags, String8 title)
{
  ...
  //- tec: early detection of composition
  {
    BOOL enabled = 0;
    DwmIsCompositionEnabled(&enabled);
    window->custom_border_composition_enabled = enabled;
...
////////////////////////////////
//~ tec: @os_hooks Monitors (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Events (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Cursors (Implemented Per-OS)
...
////////////////////////////////
//~ tec: @os_hooks Native User-Facing Graphical Messages (Implemented Per-OS)
